...
package org.nervousync.beans.header;
...
/**
 * <h2 class="en-US">Parsed information of Content-Type from the http response header</h2>
...
public final class ContentType {
	 ...
	 *                        <span class="zh-CN">数据类型</span>
	 */
	private ContentType(final String charsetEncoding, final StringType stringType) {
		this.charsetEncoding = StringUtils.isEmpty(charsetEncoding) ? Globals.DEFAULT_ENCODING : charsetEncoding;
		this.stringType = (stringType == null) ? StringType.SERIALIZABLE : stringType;
	}
	...
	public static ContentType parse(final String contentType) {
		if (StringUtils.isEmpty(contentType)) {
			return new ContentType(Globals.DEFAULT_ENCODING, StringType.SERIALIZABLE);
		}
		String[] items = StringUtils.tokenizeToStringArray(contentType, ";");
		StringType stringType = StringType.SERIALIZABLE;
		if (items.length > 0) {
