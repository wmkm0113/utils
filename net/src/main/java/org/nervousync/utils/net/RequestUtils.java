...
import org.nervousync.beans.entity.HttpEntity;
import org.nervousync.beans.header.ContentType;
import org.nervousync.beans.security.GeneX509TrustManager;
import org.nervousync.beans.request.RequestAttribute;
import org.nervousync.beans.request.RequestInfo;
...
public final class RequestUtils {
	...
	public static String resolveDomain(final String domainName) {
		try {
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Request_Resolve_IP_Count", domainName, inetAddresses.length);
			}
		...
		try {
			...
			return Arrays.stream(httpsURLConnection.getServerCertificates())
					.filter(serverCertificate ->
							matchDomain((X509Certificate) serverCertificate, domainName(urlAddress)))
					.findFirst()
					.orElse(null);
		} catch (IOException e) {
		...
		try {
			...
			if (requestInfo.octetStreamResponse()) {
				...
				if (String.class.equals(targetClass)) {
					...
					response = BeanUtils.stringToList(responseData, contentType.getStringType(),
							contentType.getCharsetEncoding(), targetClass);
				} else {
					response = BeanUtils.stringToObject(responseData, contentType.getStringType(),
							contentType.getCharsetEncoding(), targetClass);
				}
			}
	 ...
	 * <h3 class="zh-CN">从给定的网络地址中提取域名信息</h3>
	 *
	 * @param urlAddress <span class="en-US">url address</span>
	 *                   <span class="zh-CN">url地址</span>
	 * @return <span class="en-US">Domain name string</span>
	 * <span class="zh-CN">域名字符串</span>
	 */
	public static String domainName(final String urlAddress) {
		...
		String domainName = urlAddress.substring(beginIndex, endIndex);
	...
	public static String getAppURL(final HttpServletRequest request) {
		...
		requestUrl.append(scheme).append(request.getServerName());
		if ((Globals.HTTP_PROTOCOL.equalsIgnoreCase(scheme) && (port != 80))
				|| (Globals.SECURE_HTTP_PROTOCOL.equalsIgnoreCase(scheme) && (port != 443))) {
			requestUrl.append(':');
	 ...
	 * <span class="zh-CN">检查结果</span>
	 */
	private static boolean matchDomain(final X509Certificate x509Certificate, final String domainName) {
		...
		try {
			...
			if (x509Certificate.getVersion() == 3) {
				...
				if (collection != null) {
					boolean matchResult;
					if (StringUtils.isIPv4Address(domainName) || StringUtils.isIPv6Address(domainName)) {
