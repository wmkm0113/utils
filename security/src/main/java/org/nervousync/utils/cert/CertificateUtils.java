 ...
 */
package org.nervousync.utils.cert;
...
public final class CertificateUtils {
	...
	public static PublicKey publicKey(final String algorithm, final byte[] keyBytes) {
		try {
			return KeyFactory.getInstance(algorithm, "BC").generatePublic(new X509EncodedKeySpec(keyBytes));
	...
	public static PrivateKey privateKey(final String algorithm, final byte[] keyBytes) {
		...
		try {
			return KeyFactory.getInstance(algorithm, "BC").generatePrivate(new PKCS8EncodedKeySpec(keyBytes));
		} catch (InvalidKeySpecException | NoSuchAlgorithmException | NoSuchProviderException e) {
			LOGGER.error("Data_Generate_Key_Error");
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Stack_Message_Error", e);
