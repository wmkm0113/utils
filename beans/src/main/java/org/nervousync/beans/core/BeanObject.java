 ...
 * <span class="en-US">
 * <p>If JavaBean class extends the current abstract class, it can easily convert the object to JSON/XML/YAML string.
...
public abstract class BeanObject implements Serializable, Cloneable {
	...
	public final String toXml() {
		return Optional.ofNullable(this.getClass().getAnnotation(OutputConfig.class))
				.map(outputConfig ->
						this.toString(StringType.XML, outputConfig.formatted(), outputConfig.encoding()))
				.orElse(this.toString(StringType.XML, Boolean.TRUE, Globals.DEFAULT_ENCODING));
	}
	...
	public final String toJson() {
		return Optional.ofNullable(this.getClass().getAnnotation(OutputConfig.class))
				.map(outputConfig ->
						this.toString(StringType.JSON, outputConfig.formatted(), outputConfig.encoding()))
				.orElse(this.toString(StringType.JSON, Boolean.TRUE, Globals.DEFAULT_ENCODING));
	}
	...
	public final String toYaml() {
		return Optional.ofNullable(this.getClass().getAnnotation(OutputConfig.class))
				.map(outputConfig ->
						this.toString(StringType.YAML, outputConfig.formatted(), outputConfig.encoding()))
				.orElse(this.toString(StringType.YAML, Boolean.TRUE, Globals.DEFAULT_ENCODING));
	}
	...
	public final String toString() {
		return this.toString(StringType.SERIALIZABLE, Boolean.FALSE, Globals.DEFAULT_ENCODING);
	}
	 ...
	 */
	public final boolean validate(final StringType stringType) {
	 ...
	 */
	private String toString(final StringType stringType, final boolean formatOutput, final String encoding) {
		...
		switch (stringType) {
			...
			case YAML:
			case SERIALIZABLE:
				return BeanUtils.objectToString(this, stringType, formatOutput, Boolean.TRUE, encoding);
			default:
	 ...
	 */
	private String signature(final StringType stringType, final String fieldName) {
	 ...
	 * <span class="zh-CN">验证结果</span>
	 */
	private boolean verify(final String signature, final StringType stringType, final String fieldName) {
		if (StringUtils.isEmpty(signature)) {
			return Boolean.FALSE;
		}
		String[] ignoreFields = Optional.ofNullable(this.getClass().getAnnotation(JsonIgnoreProperties.class))
	...
	private TreeMap<String, Object> fieldsMap(final StringType stringType, final String... ignoreFields) {
