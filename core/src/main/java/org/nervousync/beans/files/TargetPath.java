...
/**
 * <h2 class="en-US">Compress file entry path defines</h2>
 * <h2 class="zh-CN">压缩包内文件路径定义</h2>
 *
...
public final class TargetPath {
	...
	public static TargetPath parse(final String resourceLocation) {
		if (StringUtils.containsIgnoreCase(resourceLocation, Globals.JAR_URL_SEPARATOR)) {
			int index = resourceLocation.indexOf(Globals.JAR_URL_SEPARATOR);
			return new TargetPath(resourceLocation.substring(0, index),
					resourceLocation.substring(index + Globals.JAR_URL_SEPARATOR.length()));
		}
		return null;
	}
