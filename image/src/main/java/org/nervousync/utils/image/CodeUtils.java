...
public final class CodeUtils {
	...
	public static boolean generate(final String contents, final CodeOptions codeOptions,
		...
		return Optional.ofNullable(CodeUtils.generate(contents, codeOptions))
				.map(bufferedImage -> {
					try {
						ImageIO.write(bufferedImage, codeOptions == null ? "JPEG" : codeOptions.getFileFormat(),
								outputStream);
						return Boolean.TRUE;
					} catch (IOException e) {
						return Boolean.FALSE;
	...
	public static BufferedImage generate(final String contents, final CodeOptions codeOptions) {
		...
		try {
			...
			switch (codeOptions.getCodeFormat()) {
				case QR_CODE:
					hints.put(EncodeHintType.ERROR_CORRECTION, codeOptions.getErrorLevel());
					break;
				case AZTEC:
					hints.put(EncodeHintType.ERROR_CORRECTION, codeOptions.getCorrectionLevel());
					break;
			}
			...
			if (BarcodeFormat.QR_CODE.equals(codeOptions.getCodeFormat())) {
				...
				bufferedImage = MatrixToImageWriter.toBufferedImage(bitMatrix,
						new MatrixToImageConfig(codeOptions.getOnColor().getRGB(), codeOptions.getOffColor().getRGB()));
			}
			return bufferedImage;
		} catch (Exception e) {
			if (LOGGER.isDebugEnabled()) {
